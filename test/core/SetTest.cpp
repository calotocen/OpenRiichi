...
namespace openriichi
{
	...
	/// コンストラクタ (対子用) をテストする。
	TEST_F(SetTest, testConstructorWithPair)
	{
		Set pair(P5, P5R);
	...
	TEST_F(SetTest, testConstructorWithChowOrPung)
	{
		Set chow(S4, S5R, S6);
		Set pung(S5, S5R, S5R);
	...
	/// コンストラクタ (槓子用) をテストする。
	TEST_F(SetTest, testConstructorWithKong)
	{
	...
	/// == 演算子をテストする。
	TEST_F(SetTest, testOperatorEqualTo)
	{
		// 自身と比較する。
	...
	/// != 演算子をテストする。
	TEST_F(SetTest, testOperatorNotEqualTo)
	{
		// 自身と比較する。
	...
	TEST_F(SetTest, testToGetArrangement)
	{
	...
	TEST_F(SetTest, testToGetMeldType)
	{
	...
	TEST_F(SetTest, testToGetMeldedKongType)
	{
	...
	TEST_F(SetTest, testToIsConcealed)
	{
